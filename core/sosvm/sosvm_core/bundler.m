...
%   the dual.
    ...
    if nargin == 0
      ...
      state.quadProgOpts = optimset('Algorithm', 'interior-point-convex', 'Display', 'off', 'MaxIter', 100000);
      return;
    end
    ...
    % solve quad prog
    [state.dualVariables, state.dualObjective, state.exitFlag] = quadprog(state.K, -state.f, ...

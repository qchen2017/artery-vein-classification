...
function [Gout, labelling, ground_truth] = classify_arteries_and_veins(Gout, model)
    ...
    % have size = 1)
    [patterns, ~] = encodeTrainingData({Gout});
    patterns = patterns{1};
    ...
    unary_potentials = zeros(size(unary_features, 1), 2);
    
    % scores for veins
    unary_potentials(:,1) = (w_unary(1, :) * unary_features')'; % veins
    % scores for arteries
    unary_potentials(:,2) = (w_unary(2, :) * unary_features')'; % arteries    
